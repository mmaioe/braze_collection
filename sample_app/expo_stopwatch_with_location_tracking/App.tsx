...
export default function App() {
  ...
  Braze.changeUser("stopwatch_test_user_v4")
  ...
  const showLocationTrackingPermissionDialog = ()=>{
    Location.requestForegroundPermissionsAsync();
  };

  return (
    <View style={styles.container}>
      <Text >ここにバナーの画像を入れる。With Location Tracking {topBanner?.extras["banner_img_url"]} </Text>
      <Text >{"\n"}{"\n"}</Text>
      ...
      <ButtonMenu>
        ...
        <TouchableOpacity
          onPress={showLocationTrackingPermissionDialog}
          style={[ styles.button ]}
          activeOpacity={0.8}
        >
          <View style={styles.buttonBorder}>
            <Text style={[ styles.buttonTitle]}>位置情報取得の許諾ポップアップを表示{"\n"}Show dialog for requesting location permission</Text>
          </View>
        </TouchableOpacity>
      </ButtonMenu>
    </View>
